...
<?php
    ...
    $view2=mysqli_query($conn,"select tb_jadwal.id_kelas, tb_kelas.kelas from tb_jadwal, tb_kelas where id_jadwal='$id_jadwal' AND tb_jadwal.id_kelas=tb_kelas.id_kelas") or die (mysqli_error($conn));
    $row2=mysqli_fetch_array($view2);
    $kelas=$row2['kelas'];
    $kelasid=$row2['id_kelas'];
    $view4=mysqli_query($conn,"select tb_absensih.tanggal, tb_absensih.nis,tb_siswa.id_kelas from tb_absensih, tb_siswa where tb_absensih.nis=tb_siswa.nis AND tb_absensih.tanggal='$tanggal1' AND tb_siswa.id_kelas='$kelasid'");
    ...
    <?php
        if (@$_POST['input']){
            ...
            for($i=0;$i<$no;$i++){
                ...
                $query=mysqli_query($conn,"insert into tb_absensi(id_jadwal, tanggal, nis, ket) values('$id_jadwal','$tanggal','$nis','$ket')") or die (mysqli_error($conn));
            ...
            if($cek==0){
                for($a=0;$a<$no;$a++){
                    ...
                    $query1=mysqli_query($conn,"insert into tb_absensih(tanggal, nis, ket) values('$tanggal','$nis','$ket')") or die (mysqli_error($conn));
                }
            }
